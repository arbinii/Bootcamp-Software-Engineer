...
int totalBalance() {
  ...
  for (var i = 0; i < transactions.length; i++) {
    listAmount.add(transactions[i].type == 'Pemasukan'
...
List<Transaction> getExpenseTransactionToday() {
  ...
  for (var i = 0; i < transactions.length; i++) {
    if (transactions[i].category.type == 'Pengeluaran' &&
...
String formatCurrency(int value) {
  final format = NumberFormat.currency(
    ...
    locale: 'id_IDR', // Vietnamese locale for formatting
  );

  return 'Rp ${format.format(value)}'; // Append ' vnđ' to the formatted value
}
